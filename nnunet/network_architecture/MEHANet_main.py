...
class MEHANet(SegmentationNetwork):
    ...
    def __init__(self, input_channels, base_num_features, num_classes, num_pool, num_conv_per_stage=2,
                 ...
                 seg_output_use_bias=False,input_resolution=[48,192,192],heads=[8,8,8], blocks=3):
        """
        ...
        for d in range(num_pool):
            ...
            if d == 0:
                self.conv_blocks_context.append(DoubleConv(input_features, output_features, stride=1, kernel_size=3))
                divided_size =np.array([1,1,1])
            ...
            # add convolutions
            if d != 0:
                self.conv_blocks_context.append(make_res_layer(input_features, output_features, blocks, first_stride, mamba_layer=MambaLayer(dim=output_features)))
            if d> 2:
        ...
        # print(self.conv_blocks_context[-2][1].conv2.out_channels)
        # print(self.conv_blocks_context[-3][1].conv2.out_channels)
        # print(self.conv_blocks_context[-4][1].conv2.out_channels)
        # print(self.conv_blocks_context[-5][1].conv2.out_channels)
       # print(self.conv_blocks_context[-6][1].conv2.out_channels)
        # now lets build the localization pathway 解码器
        for u in range(num_pool):
            nfeatures_from_down = final_num_features
            if u != 4:
                nfeatures_from_skip = self.conv_blocks_context[-(2+u)][1].conv2.out_channels  # self.conv_blocks_context[-1] is bottleneck, so start with -2
            else:
                nfeatures_from_skip =self.conv_blocks_context[0].conv[0].out_channels
                print(nfeatures_from_skip)
...
if __name__ == '__main__':
    ...
    NetS = MEHANet(input_channels=1, base_num_features=32, num_classes=3, num_pool=5, num_conv_per_stage=2,
                 feat_map_mul_on_downscale=2, conv_op=nn.Conv3d,
